...
export default function TeamCard({ 
  id, name, description, members, activeProjects, completedTasks, color, onInvite 
}: TeamCardProps) {
  const colorClasses = {
        ...
        <div className="flex -space-x-2">
          {members.slice(0, 5).map((member, index) => (
            <div key={member.id} className="relative">
              ...
              <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-gray-800 ${statusColors[member.status]}`}></div>
            </div>
          ))}
          {members.length > 5 && (
            <div className="w-8 h-8 bg-gray-600 rounded-full flex items-center justify-center text-white font-medium text-xs border-2 border-gray-800">
              +{members.length - 5}
            </div>
          )}
        </div>
